# FASE 2: Vender Servicios y Automatizaciones a los Partners de Auvesta

## Resumen de la Fase

La segunda fase consiste en **vender tus servicios de automatizacion y marketing digital a los partners de Auvesta**. Ya conoces la red, ya tienes las herramientas funcionando, ahora ofreces esas mismas soluciones a otros que las necesitan.

**Objetivo principal:** Generar ingresos adicionales vendiendo servicios tecnicos a la red de partners, mientras tambien captas gente para tu grupo y aprovechas sus ventas.

---

## Estrategia General

```
┌─────────────────────┐
│  Partners de        │
│  Auvesta            │
│                     │
│  - Landing mala     │
│  - Sin RRSS         │
│  - Sin sistema      │
│  - Necesitan        │
│    clientes         │
└────────┬────────────┘
         │
         v
┌─────────────────────┐
│  TU OFERTA          │
│                     │
│  - Arreglar landing │
│  - Gestionar RRSS   │
│  - Automatizaciones │
│  - Captacion con IA │
│  - Meter gente en   │
│    su grupo/red     │
└────────┬────────────┘
         │
         v
┌─────────────────────┐
│  RESULTADO          │
│                     │
│  - Cobras servicios │
│  - Metes gente en   │
│    tu grupo         │
│  - Aprovechas sus   │
│    ventas tambien   │
│  - Creces la red    │
└─────────────────────┘
```

---

## Identificar Partners Potenciales

### Donde Encontrarlos

1. **Dentro de Auvesta** - Otros vendedores/participantes de la red
2. **Eventos de Auvesta** - Reuniones, formaciones, encuentros
3. **Grupos de WhatsApp/Telegram** - Comunidades de la red
4. **Redes sociales** - Partners que publican sobre Auvesta

### Criterios para Identificar Buenos Candidatos

| Criterio | Senal de que Necesitan Ayuda |
|---|---|
| Landing page | No tienen o es muy basica/fea |
| Redes sociales | Perfil abandonado o sin estrategia |
| Automatizaciones | Hacen todo manual |
| Captacion | No saben como encontrar clientes |
| Grupo/Red | Tienen pocos referidos, no crecen |

### Proceso de Cualificacion

1. **Observar** - Mira sus perfiles, landing pages, actividad
2. **Evaluar** - Identifica que problemas concretos tienen
3. **Registrar** - Mete el lead en Airtable para clasificar con IA
4. **Priorizar** - La automatizacion te dice a quien contactar primero
5. **Proponer** - Contacta con una solucion concreta

---

## Servicios que Ofreces a los Partners

### 1. Mejora de Landing Pages

**Problema comun:** Landing pages feas, lentas, sin conversion.

**Lo que ofreces:**
- Redisenar la landing para que convierta
- Optimizar para movil
- Mejorar textos (copywriting)
- Anadir formularios de captacion
- Conectar con automatizaciones

**Precio orientativo:** 200-500 EUR por landing

### 2. Gestion de Redes Sociales

**Problema comun:** Perfiles abandonados o sin estrategia.

**Lo que ofreces:**
- Estrategia de contenido
- Creacion de posts (usando IA para generar ideas)
- Publicacion regular
- Interaccion con seguidores
- Reportes mensuales

**Precio orientativo:** 150-300 EUR/mes

### 3. Automatizaciones Make.com

**Problema comun:** Hacen todo manual, pierden tiempo.

**Lo que ofreces:**
- Clasificacion automatica de leads (Airtable + Claude)
- Envio automatico de mensajes de seguimiento
- Recordatorios de contacto
- Reportes automaticos de ventas
- Integracion entre herramientas

**Precio orientativo:** 100-300 EUR por automatizacion

### 4. Captacion de Clientes con IA

**Problema comun:** No saben como encontrar clientes nuevos.

**Lo que ofreces:**
- Sistema de captacion automatizada
- Clasificacion inteligente de leads
- Estrategia de contenido generado con IA
- Secuencias de contacto automatizadas
- Metricas y optimizacion continua

**Precio orientativo:** 200-400 EUR/mes

---

## Estrategia de Crecimiento del Grupo

### Que es "El Grupo" y Como Funciona

El grupo es tu **equipo/downline dentro de la estructura de Auvesta**. Cuando metes gente en tu grupo:

- Ellos se registran en Auvesta bajo tu enlace de referido
- Cuando ellos venden productos de Auvesta, tu recibes una **comision de equipo** (% sobre sus ventas)
- No les cobras nada. Auvesta paga esa comision de su margen
- Cuantas mas personas activas tenga tu grupo, mas ingresos pasivos generas

### Mecanica de Atribucion de Ventas

```
┌─────────────────────────────────────────────────────────────┐
│           COMO SE ATRIBUYEN LAS VENTAS DEL GRUPO            │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  1. Tu registras a un nuevo partner con tu enlace personal  │
│     (Auvesta asigna ese partner a tu estructura)            │
│                                                             │
│  2. El partner hace una venta de productos Auvesta          │
│                                                             │
│  3. Auvesta registra la venta y calcula comisiones:         │
│     - El partner cobra su comision directa (venta propia)   │
│     - TU cobras comision de equipo (% del volumen)          │
│                                                             │
│  4. El pago llega automaticamente desde Auvesta             │
│     (no tienes que facturar ni cobrar tu al partner)        │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### Herramienta de Seguimiento del Grupo

| Herramienta | Funcion | Detalle |
|---|---|---|
| **Panel de Auvesta** | Registro oficial de ventas y comisiones | Aqui ves las ventas de tu equipo y tus comisiones generadas |
| **Airtable (CRM propio)** | Seguimiento personal de actividad | Registra quien esta en tu grupo, su actividad, y quien necesita apoyo |
| **WhatsApp grupo** | Comunicacion diaria con tu equipo | Compartir tips, motivacion, novedades, formacion rapida |

### Proceso Concreto para Gestionar tu Grupo

1. **Captar:** Usa tus servicios como puerta de entrada. Al ayudar a un partner con su landing o RRSS, le propones unirse a tu equipo
2. **Registrar:** El partner se registra en Auvesta con tu enlace de referido personal
3. **Verificar:** Confirma en el panel de Auvesta que aparece bajo tu estructura
4. **Apuntar en Airtable:** Crea un registro con: nombre, fecha de entrada, nivel de actividad, ultimo contacto
5. **Formar:** Ensenarle a vender (comparte tus automatizaciones, scripts, procesos)
6. **Hacer seguimiento:** Revisa semanalmente quien esta vendiendo y quien no. Contacta a los inactivos
7. **Cobrar:** Las comisiones de equipo se acumulan y pagan segun el calendario de Auvesta

### Modelo de Comisiones de Equipo (Referencia)

| Nivel | Descripcion | Comision Estimada |
|---|---|---|
| Nivel 1 (directos) | Personas que tu registraste directamente | 5-10% del volumen de sus ventas |
| Nivel 2+ (indirectos) | Personas que registraron tus directos | 2-5% (segun plan de compensacion Auvesta) |

> **Nota:** Los porcentajes exactos dependen del plan de compensacion vigente de Auvesta.
> Consulta la documentacion oficial de Auvesta para los numeros actualizados.
> Los KPIs de esta fase usan estimaciones conservadoras.

### Meter Gente en Tu Grupo - Paso a Paso

1. **Ofrecer valor primero** - Ayuda a los partners con sus problemas (landing, RRSS, automatizaciones)
2. **Demostrar resultados** - Muestra que tus automatizaciones funcionan y que vendes bien
3. **Invitar al grupo** - Proponer que se registren bajo tu enlace en Auvesta
4. **Formar a los nuevos** - Ensenales a usar las herramientas y a vender
5. **Aprovechar sus ventas** - Las comisiones de equipo fluyen automaticamente via Auvesta

### Canales para Captar Gente para el Grupo

| Canal | Accion | Objetivo |
|---|---|---|
| Servicios previos | Ofrecer tus servicios primero, proponer unirse despues | Confianza ya establecida |
| WhatsApp | Compartir resultados y casos de exito | Generar interes |
| Eventos Auvesta | Presentar tus automatizaciones en vivo | Demostrar valor |
| Redes sociales | Publicar contenido educativo | Atraer curiosos |
| Boca a boca | Pedir referidos a partners satisfechos | Crecimiento organico |
| Landing page | Crear pagina especifica para unirse | Captar leads 24/7 |

### Beneficios de Tener un Grupo Grande

- **Ingresos pasivos via Auvesta** - Comisiones de equipo sobre las ventas de tus miembros (sin cobrarte tu a ellos)
- **Efecto red** - Mas gente = mas visibilidad = mas gente interesada en unirse
- **Economia de escala** - Las automatizaciones que creas sirven a todo tu equipo
- **Datos** - Mas leads en tu sistema = mejor rendimiento del clasificador IA
- **Autoridad** - Un grupo grande te posiciona como lider/referente dentro de Auvesta
- **Doble ingreso** - Cobras por los servicios que les vendes (Fase 2) Y por sus ventas de Auvesta (comision equipo)

---

## Plan de Accion Mensual

### Mes 1: Identificacion y Primeros Contactos
- [ ] Listar 20 partners potenciales de la red Auvesta
- [ ] Analizar sus landing pages y redes sociales
- [ ] Registrar en Airtable y clasificar con IA
- [ ] Contactar los 5 mas calientes
- [ ] Cerrar al menos 1 cliente de servicios
- [ ] Meter al menos 3 personas nuevas en tu grupo

### Mes 2: Primeros Servicios y Crecimiento
- [ ] Entregar primer servicio (landing o RRSS)
- [ ] Pedir testimonio/referido al primer cliente
- [ ] Contactar siguientes 10 partners
- [ ] Cerrar 2-3 clientes mas
- [ ] Meter 5-10 personas en tu grupo
- [ ] Documentar resultados para usar como caso de exito

### Mes 3: Escalado y Sistemas
- [ ] Tener 5+ clientes activos de servicios
- [ ] Tener 15+ personas en tu grupo
- [ ] Crear paquetes de servicios estandarizados
- [ ] Automatizar onboarding de nuevos clientes
- [ ] Empezar a generar ingresos de las ventas del grupo
- [ ] Preparar transicion a Fase 3

---

## Objetivos Economicos Fase 2

| Concepto | Mes 1 | Mes 3 | Mes 6 |
|---|---|---|---|
| Clientes de servicios | 1-2 | 5 | 10 |
| Ingresos por servicios | 300 EUR | 1.500 EUR | 3.000 EUR |
| Personas en tu grupo | 3-5 | 15 | 30+ |
| Ingresos por red/grupo | 100 EUR | 500 EUR | 1.500 EUR |
| **Total Fase 2** | **400 EUR** | **2.000 EUR** | **4.500 EUR** |

*Nota: Estos ingresos se suman a los de Fase 1 (ventas directas para Auvesta)*

---

## Herramientas Adicionales para esta Fase

| Herramienta | Uso | Coste |
|---|---|---|
| Make.com (upgrade) | Mas operaciones por mas clientes | 16 EUR/mes |
| Airtable (pro) | Mas registros y vistas | 20 EUR/mes |
| Claude API | Mas clasificaciones | 10-20 EUR/mes |
| Hosting | Para las landing pages de clientes | 5-10 EUR/mes |
| Canva Pro | Crear contenido para clientes | 12 EUR/mes |

---

## Transicion a Fase 3

Cuando la Fase 2 funciona bien:

1. **Ya tienes sistemas probados** - Automatizaciones que funcionan para otros
2. **Ya tienes portfolio** - Casos de exito con partners
3. **Ya tienes ingresos estables** - Servicios + red/grupo
4. **Aplica todo a ti mismo** - Usa exactamente lo mismo para kepa-tecnologia

La Fase 2 te demuestra que tus servicios tienen valor en el mercado. La Fase 3 es usarlos para tu propio negocio.
